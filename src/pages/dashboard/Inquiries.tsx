...
const InquiriesPage = () => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    if (!isManagerOrAbove()) {
      toast.error("You don't have permission to update inquiries");
      return;
    }
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((inq) => inq.status === activeTab);
  }, [inquiries, activeTab]);
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Review and respond to service inquiries from clients
        </p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList variant="pills" className="mb-6">
          <TabsTrigger variant="pills" value="all">
            All {counts.all > 0 && <span className="ml-1.5 text-xs">({counts.all})</span>}
          </TabsTrigger>
          <TabsTrigger variant="pills" value="new">
            New {counts.new > 0 && <span className="ml-1.5 text-xs">({counts.new})</span>}
          </TabsTrigger>
          <TabsTrigger variant="pills" value="in_progress">
            In Progress {counts.in_progress > 0 && <span className="ml-1.5 text-xs">({counts.in_progress})</span>}
          </TabsTrigger>
          <TabsTrigger variant="pills" value="closed">
            Resolved {counts.closed > 0 && <span className="ml-1.5 text-xs">({counts.closed})</span>}
          </TabsTrigger>
        </TabsList>
