...
export function NavBarItem({ name, route, closeDrawer }: { name: string, route: string, closeDrawer?: () => void }) {
    ...
    let button;
    if (pathname == "/") {
        button = <a className="text-[10px] font-medium md:text-[1.4vw] lg:text-[16px] text-[#006847] p-[0.5vw] hover:bg-gray-200 rounded-sm" onClick={handleOnClickNoRouteChange}>{name}</a>
    }
    else {
        button = <a className="text-[10px] font-medium md:text-[1.4vw] lg:text-[16px] text-[#006847] p-[0.5vw] hover:bg-gray-200 rounded-sm" onClick={handleOnClick}>{name}</a>
    }
