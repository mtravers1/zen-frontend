...
import logo from '../../../public/zentavos-logo.png'
...
const DashboardSidebar = () => {
	...
	const clientsItems: MenuItem[] = [
		...
		{ title: "Subscriptions", url: "/dashboard/clients/subscriptions", icon: CreditCard },
		{ title: "Messaging", url: "/dashboard/clients/messaging", icon: MessageSquare },
		{ title: "Assignments", url: "/dashboard/clients/assignments", icon: UserCheck },
	];
	...
	const documentsItems: MenuItem[] = [
		...
		{ title: "Internal Documents", url: "/dashboard/documents/internal", icon: Lock },
	];
	...
	const settingsItems: MenuItem[] = [
		...
		{ title: "Roles & Permissions", url: "/dashboard/settings/roles", icon: ShieldCheck },
	...
	const renderMenuItem = (item: MenuItem) => (
		<SidebarMenuItem key={item.title}>
			<SidebarMenuButton asChild>
				<NavLink
					href={item.url}
					className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors relative ${isActive(item.url)
		...
		return (
			<Collapsible open={isOpen} onOpenChange={() => toggleSection(sectionKey)}>
				<CollapsibleTrigger className="w-full">
					<div
						className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors cursor-pointer ${hasActiveItem
							...
							{items.map((item) => (
								<SidebarMenuItem key={item.title}>
									<SidebarMenuButton asChild>
										<NavLink
											href={item.url}
											className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sm ${isActive(item.url)
				...
				{!collapsed && (
					<Link href="/" className="flex items-center gap-2 overflow-hidden">
						<Image
							...
							width={200}
							alt="Zentavos"
							className="h-32 w-32 object-contain logo-dark-mode shrink-0"
						/>
